...
data=pd.read_csv('countycases-GA.csv')
...
for i in range(1,144):
    Country.append(DATA.iloc[i+390,2])
    Pos.append(float(DATA.iloc[i+390,-1]))
...
for i in range(1,144):
    ...
    A_POS3.append(int((float((int(DATA.iloc[i+388,-1]))-int(DATA.iloc[i+388,-3]))/3/(int(DATA.iloc[i + 388, -1])))*100000))
    ...
    A_POS5.append(int((float((int(DATA.iloc[i+388,-1]))-int(DATA.iloc[i+388,-5]))/5/(int(DATA.iloc[i + 388, -1])))*100000))
    ...
    A_POS7.append(int((float((int(DATA.iloc[i+388,-1]))-int(DATA.iloc[i+388,-7]))/7/(int(DATA.iloc[i + 388, -1])))*100000))
...
for i in range(0,143):
    ...
    NUM.append(A_POS7[i])
    NUM.append(A_POS5[i])
data1=[list(z) for z in zip(Country,A_POS3)]
data2=[list(z) for z in zip(Country,A_POS5)]
data3=[list(z) for z in zip(Country,A_POS7)]
pos={}
...
time="2020/{}Georgia感染增加数（设定各县当天感染人数均达到100,000人）".format(DATA.columns.values[-1])
tl = Timeline()
i=1
a=0
        ...
        Map(init_opts=opts.InitOpts(width="1400px", height="800px"))
         ...
         .add(
            ...
            title_opts=opts.TitleOpts(
                title=time,
                subtitle="近七天GA州感染人数增长速度:{}   近五天:{}  近三天:{}".format(account7,account5,account3),
                pos_top="4%",
                pos_left="30%"
            ),
