...
death_name='D:/Pchong/data_see/data_all/20200531-world-death-data.json.csv'
positive_name='D:/Pchong/data_see/data_all/20200531-world-confirm-data.json.csv'
recovered_name='D:/Pchong/data_see/data_all/20200531-world-cover-data.json.csv'
...
for i in range(1,len(line_world.columns.values)-1):
    ...
    if line_world.columns.values[i]=='World':
        ...
        break
pos_world=[]
for j in range(1,len(line_world)):
    pos_world.append(line_world.iloc[j,i])
    line_world.iloc[j,i]=0              
print(line_world.iloc[j,i])
print(line_world.iloc[j,i])
print(type(line_world.columns.values[1]))
for i in range(1,len(line_world)):
    Time_POS[i-1]=[]
    pos=[]
    a=0
    data2[i-1]=[]
...
for i in range(0, len(line_world)-1):
    map0 = (
        Map()
        .add("确诊人数", Time_POS[i], "world",is_map_symbol_show=False)
        .set_series_opts(label_opts=opts.LabelOpts(is_show=False))
        .set_global_opts(
            title_opts=opts.TitleOpts(title="{}-世界疫情趋势".format(line_date[i]),subtitle="世界确诊人数总和{}".format(pos_world[i])),
