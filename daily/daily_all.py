...
name1 = 'D:/Pchong/data_see/data/20200628-data/2020-06-27-1point3acres-us_state-confirmed-data.json.csv'
name2 = 'D:/Pchong/data_see/data/20200628-data/2020-06-27-1point3acres-us_state-death-data.json.csv'
name3 = 'D:/Pchong/data_see/data_all/20200627-world-confirm-data.json.csv'
...
for i in range(0, int(daily_country.shape[1])):
    ...
    if daily_country.iloc[1, i] == 'India':
        print('India', i)
    if daily_country.iloc[1, i] == 'Japan':
        print('Japan', i)
    if daily_country.iloc[1, i] == 'Iran':
        print('Iran', i)
    if daily_country.iloc[1, i] == 'Italy':
        print('Italy', i)
    if daily_country.iloc[1, i] == 'Spain':
        print('Spain', i)
    if daily_country.iloc[1, i] == 'France':
        print('France', i)
...
for i in range(2, len(daily_country)-1):
    ...
    if math.isnan(float(daily_country.iloc[i, 77])):  # Japan
        daily_country.iloc[i, 77] = 0
        daily_country.iloc[i+1, 77] = 0
    if math.isnan(float(daily_country.iloc[i, 153])):  # US
        daily_country.iloc[i, 153] = 0
        daily_country.iloc[i+1, 153] = 0
    if math.isnan(float(daily_country.iloc[i, 163])):  # World
        daily_country.iloc[i, 163] = 0
        daily_country.iloc[i+1, 163] = 0
    if math.isnan(float(daily_country.iloc[i, 8])):  # Australia
        daily_country.iloc[i, 8] = 0
        daily_country.iloc[i+1, 8] = 0
    if math.isnan(float(daily_country.iloc[i, 125])):  # Russia
        daily_country.iloc[i, 125] = 0
        daily_country.iloc[i+1, 125] = 0
    ...
    if math.isnan(float(daily_country.iloc[i, 71])):  # Iran
        daily_country.iloc[i, 71] = 0
        daily_country.iloc[i+1, 71] = 0
    if math.isnan(float(daily_country.iloc[i, 75])):  # Italy
        daily_country.iloc[i, 75] = 0
        daily_country.iloc[i+1, 75] = 0
    if math.isnan(float(daily_country.iloc[i, 139])):  # Spain
        daily_country.iloc[i, 139] = 0
        daily_country.iloc[i+1, 139] = 0
    if math.isnan(float(daily_country.iloc[i, 54])):  # France
        daily_country.iloc[i, 54] = 0
        daily_country.iloc[i+1, 54] = 0
    China.append(
        float(daily_country.iloc[i+1, 32])-float(daily_country.iloc[i, 32]))
    Japan.append(
        float(daily_country.iloc[i+1, 77])-float(daily_country.iloc[i, 77]))
    USA.append(
        float(daily_country.iloc[i+1, 153])-float(daily_country.iloc[i, 153]))
    World.append(
        float(daily_country.iloc[i+1, 163])-float(daily_country.iloc[i, 163]))
    Australia.append(
        float(daily_country.iloc[i+1, 8])-float(daily_country.iloc[i, 8]))
    Russia.append(
        float(daily_country.iloc[i+1, 125])-float(daily_country.iloc[i, 125]))
    Germany.append(
    ...
    Iran.append(
        float(daily_country.iloc[i+1, 71])-float(daily_country.iloc[i, 71]))
    Italy.append(
        float(daily_country.iloc[i+1, 75])-float(daily_country.iloc[i, 75]))
    Spain.append(
        float(daily_country.iloc[i+1, 139])-float(daily_country.iloc[i, 139]))
    France.append(
        float(daily_country.iloc[i+1, 54])-float(daily_country.iloc[i, 54]))
...
BRAZIL_all = []
for i in range(2, len(daily_country)):
    CHINA_all.append(daily_country.iloc[i, 32])
    JAPAN_all.append(daily_country.iloc[i, 77])
    USA_all.append(daily_country.iloc[i, 153])
    RUSSIA_all.append(daily_country.iloc[i, 125])
    Brazil.append(float(daily_country.iloc[i, 19]))
    GERAMNY_all.append(daily_country.iloc[i, 57])
...
a = 0
for i in range(2, len(daily_country)-1):
    a = float(daily_country.iloc[i+1, 163]) / \
        float(daily_country.iloc[i, 163])
    accoun_world.append('%.2f' % a)
    if daily_country.iloc[i, 32] == 0:
        accoun_china.append('0')
    else:
        a = float(daily_country.iloc[i+1, 32]) / \
            float(daily_country.iloc[i, 32])
        accoun_china.append('%.2f' % a)
    if daily_country.iloc[i, 77] == 0:
        accoun_Japan.append('0')
    else:
        a = float(daily_country.iloc[i+1, 77]) / \
            float(daily_country.iloc[i, 77])
        accoun_Japan.append('%.2f' % a)
    if daily_country.iloc[i, 125] == 0:
        accoun_Germany.append('0')
    else:
        a = float(daily_country.iloc[i+1, 125]) / \
            float(daily_country.iloc[i, 125])
        accoun_Germany.append('%.2f' % a)
    if daily_country.iloc[i, 153] == 0:
        accoun_USA.append('0')
    else:
        a = float(daily_country.iloc[i+1, 153]) / \
            float(daily_country.iloc[i, 153])
        accoun_USA.append('%.2f' % a)
...
(
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title="各国确诊情况"),
        tooltip_opts=opts.TooltipOpts(
            trigger="axis", axis_pointer_type="cross"),
        yaxis_opts=opts.AxisOpts(
            type_="value",
        ...
        xaxis_opts=opts.AxisOpts(
            type_="category", boundary_gap=False, name="日期"),
    ...
    .render("D:\Pchong\data_see\html_all\pos_all.html")
)
# 各国每日新增确诊人数
...
(
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title="各国每日新增确诊人数"),
        tooltip_opts=opts.TooltipOpts(
            trigger="axis", axis_pointer_type="cross"),
        yaxis_opts=opts.AxisOpts(
            type_="value",
    ...
    .render("D:\Pchong\data_see\html_all\daily_add.html")
)
