...
url = 'https://coronavirus.1point3acres.com/_next/static/chunks/f7df1767092d89504763bcfc886ec9c2b7dbd395.8c1e8d4fea1476d41ad0.js'
headers = {
...
write_list_to_json(data_relative_confirmed_json,'202005018-world-confirm-data.json','D:/Pchong/可视化/data/20200518-data/')
data_csv=pd.DataFrame(json.loads(open('202005018-world-confirm-data.json','r+').read()))
print(end_1[36][1])
...
data_csv.T
data_csv.to_csv('202005018-world-confirm-data.json.csv')
df=pd.read_csv('202005018-world-confirm-data.json.csv')
new_csv=df.T
new_csv.to_csv('202005018-world-confirm-data.json.csv')


#死亡人数

'''
care=end_1[0][4].replace('[','').replace(']','').split(',')
try:
    time=end_1[0][5].replace('/',',').replace('/',',').replace('"','').split(',')
    print(time)
    time[2]='2020'
    date=[]
    in_date = time[2]+'-'+time[0]+'-'+time[1]
    dt = datetime.datetime.strptime(in_date, "%Y-%m-%d")
    for k in range(len(end_1[0][4].replace('[','').replace(']','').split(','))):
        out_date = (dt + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
        dt=datetime.datetime.strptime(out_date, "%Y-%m-%d")
        date.append(out_date)
    print(date)
